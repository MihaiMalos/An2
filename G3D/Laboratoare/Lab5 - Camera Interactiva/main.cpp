...
public:
	...
	void Move(EDirection direction, double deltaTime)
	{
	...
	void MouseControl(float xPos, float yPos, double deltaTime)
	{
		...
		float xChange = (xPos - lastX) * sensitivity;
		float yChange = (lastY - yPos) * sensitivity;
...
void processInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
		pCamera->Move(EDirection::Forward, deltaTime);
	}

	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
		pCamera->Move(EDirection::Backward, deltaTime);

	}

	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
		pCamera->Move(EDirection::Left, deltaTime);

	}

	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
		pCamera->Move(EDirection::Right, deltaTime);

	}

	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) {
		pCamera->Move(EDirection::Down, deltaTime);
	}

	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) {
		pCamera->Move(EDirection::Up, deltaTime);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	pCamera->MouseControl((float)xpos, (float)ypos, deltaTime);
}
